...
unix:!mac {
  ...
  QMAKE_LFLAGS += -rdynamic
    ...
    DEFINES += PROGRAM_DATA_DIR=\\\"$$PREFIX/share/NomadDictionary/\\\"
    target.path = $$PREFIX/bin/
    locale.path = $$PREFIX/share/NomadDictionary/locale/
    ...
    INSTALLS += desktops
    helps.path = $$PREFIX/share/NomadDictionary/help/
...
mac {
    ...
    !CONFIG( no_chinese_conversion_support ) {
        CONFIG += chinese_conversion_support
        CONFIG( x86 ) {
            QMAKE_POST_LINK += & mkdir -p NomadDictionary.app/Contents/MacOS/opencc & \
                                 cp -R $${PWD}/opencc/*.json NomadDictionary.app/Contents/MacOS/opencc/ & \
                                 cp -R $${PWD}/opencc/*.ocd NomadDictionary.app/Contents/MacOS/opencc/
        } else {
            QMAKE_POST_LINK += & mkdir -p NomadDictionary.app/Contents/MacOS/opencc & \
                                 cp -R $${PWD}/opencc/x64/*.json NomadDictionary.app/Contents/MacOS/opencc/ & \
                                 cp -R $${PWD}/opencc/x64/*.ocd NomadDictionary.app/Contents/MacOS/opencc/
        }
    }
}
